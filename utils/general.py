...
def labels_to_class_weights(labels, nc=80):
    # Get class weights (inverse frequency) from training old_labels
    if labels[0] is None:  # no old_labels loaded
        return torch.Tensor()

    labels = np.concatenate(labels, 0)  # old_labels.shape = (866643, 5) for COCO
    classes = labels[:, 0].astype(np.int)  # old_labels = [class xywh]
    weights = np.bincount(classes, minlength=nc)  # occurrences per class

    # Prepend gridpoint count (for uCE training)
    # gpi = ((320 / 32 * np.array([1, 2, 4])) ** 2 * 3).sum()  # gridpoints per image
    # weights = np.hstack([gpi * len(old_labels)  - weights.sum() * 9, weights * 9]) ** 0.5  # prepend gridpoints to start
...
def segments2boxes(segments):
    # Convert segment old_labels to box old_labels, i.e. (cls, xy1, xy2, ...) to (cls, xywh)
    boxes = []
    for s in segments:
        x, y = s.T  # segment xy
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    multi_label &= nc > 1  # multiple old_labels per box (adds 0.5ms/img)
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Cat apriori old_labels if autolabelling
