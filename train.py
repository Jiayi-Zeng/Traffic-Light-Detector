...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    assert mlc < nc, f'Label class {mlc} exceeds nc={nc} in {data}. Possible class old_labels are 0-{nc - 1}'
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            ...
            # c = torch.tensor(old_labels[:, 0])  # classes
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%10s' * 7) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'old_labels', 'img_size'))
        if RANK in [-1, 0]:
