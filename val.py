    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out):
            ...
            if nl:
                ...
                scale_coords(im[si].shape[1:], tbox, shape, shapes[si][1])  # native-space old_labels
                labelsn = torch.cat((labels[:, 0:1], tbox), 1)  # native-space old_labels
            ...
            # Save/log
            if save_txt:
                save_one_txt(predn, save_conf, shape, file=save_dir / 'old_labels' / (path.stem + '.txt'))
            if save_json:
        ...
        if plots and batch_i < 3:
            f = save_dir / f'val_batch{batch_i}_labels.jpg'  # old_labels
